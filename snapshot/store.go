...
const (
	...
	DefaultSnapshotPath   = "today.json"
...
var ErrSnapshotStoreUnavailable = errors.New("snapshot store unavailable")
...
func NewDefaultStore() Store {
	...
	if isVercelProduction() {
		return NewUnavailableStore(BlobTokenEnvKey + " is required in production")
	}
...
func isVercelProduction() bool {
	return os.Getenv("VERCEL") == "1" || strings.EqualFold(os.Getenv("VERCEL"), "true")
}
...
func (s *blobStore) Save(ctx context.Context, classInfo *model.ClassInfo) error {
	...
	req.Header.Set("x-api-blob-request-id", s.requestID())
...
func (s *blobStore) requestID() string {
	storeID, _ := s.storeID()
	return storeID + ":" + time.Now().Format("20060102150405")
}
...
func (s *blobStore) storeID() (string, error) {
...
func (s *unavailableStore) Load(_ context.Context) (*model.ClassInfo, error) {
	return nil, errors.Join(ErrSnapshotStoreUnavailable, errors.New(s.reason))
}

func (s *unavailableStore) Save(_ context.Context, _ *model.ClassInfo) error {
	return errors.Join(ErrSnapshotStoreUnavailable, errors.New(s.reason))
}
